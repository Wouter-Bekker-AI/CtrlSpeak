...
4. **Installation flow** - The Install/repair control (or `--download-cuda-only` alias) runs `install_cuda_runtime_with_progress`, which shows a lockout window, installs NVIDIA wheels with pip when needed, and re-validates availability. When no CUDA-capable GPU is detected the button remains disabled and the CLI flag exits immediately, so CtrlSpeak stays on CPU with a clear warning.?F:utils/models.py?L253-L344?
