...
### Chat with Bot
After enabling Chat with Bot from the management window, CtrlSpeak launches the SocialRobot helper to provide a spoken dialogue loop. The bot reuses the local transcription server, forwards recognized text to the configured Ollama model, and plays responses via Kokoro TTS. You can tailor the experience by selecting an identity (see third_party/social_robot/identities/<name>), which bundles the system prompt, default voice, LLM settings, and future memory storage for the persona. Refer to docs/bot_integration.md for prerequisites, environment overrides, and automation tips.
