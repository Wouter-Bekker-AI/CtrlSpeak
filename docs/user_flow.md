...
2. **Model acquisition** – `_ensure_model_files` auto-approves the download and drives `download_model_with_gui`, displaying the full-screen progress dialog until staging is finished (with a cancel button for exceptional cases). Files are stored under `%APPDATA%/CtrlSpeak/models`.【F:utils/models.py†L805-L838】【F:utils/models.py†L368-L540】
