...
5. **Settings persistence** – The selected mode is written to `settings.json` inside the platform config directory (`%APPDATA%/CtrlSpeak/settings.json` on Windows or `${XDG_CONFIG_HOME:-~/.config}/CtrlSpeak/settings.json` on Linux/macOS). Future boots will start directly in this mode unless changed manually later.【F:utils/gui.py†L1051-L1111】【F:utils/config_paths.py†L58-L72】
...
1. **Hotkey press** – `keyboard.Listener` invokes `on_press`. When `ctrl_r` is detected and no recording is in progress, CtrlSpeak creates a unique WAV file in `${data_root}/temp`, starts a PyAudio capture thread, and displays an overlay with the live waveform sourced from the microphone.【F:utils/system.py†L600-L643】【F:utils/config_paths.py†L48-L64】
...
3. **Hotkey release** – When `ctrl_r` is released, CtrlSpeak transitions the overlay to a “Processing…” state, starts looping `loading.wav` via `start_processing_feedback`, and calls `transcribe_audio` with the recorded file. When a SocialRobot persona is active, the loop runs at 50 % volume so the feedback is softer while the conversation window is open.【F:utils/system.py†L644-L690】【F:utils/system.py†L362-L441】【F:utils/models.py†L1240-L1292】
...
5. **Chat theme preference** – The Assistants card exposes a toggleable **Theme** button that defaults to **Dark**, persists the `chat_theme` flag in `settings.json`, and now streams updates to any running SocialRobot chat window so the look flips between light and dark immediately without restarting the bot. The chat window activates itself during the switch so the Windows title bar recolors instantly, applies rounded corners when supported, and keeps matching rounded controls plus icon button sizing in both themes.【F:utils/gui.py†L1432-L1493】【F:utils/config_paths.py†L12-L24】【F:utils/bot_integration.py†L850-L908】【F:third_party/social_robot/main.py†L511-L707】【F:third_party/social_robot/ui/chat_window.py†L213-L356】【F:third_party/social_robot/ui/chat_window.py†L428-L520】
...
3. **Runtime detection** – `cuda_runtime_ready` ensures environment variables point at `${data_root}/cuda/12.3`, verifies GPU devices via ctranslate2, and, on Windows, attempts to load the required DLLs (`cudart64_12.dll`, `cublas64_12.dll`, `cublasLt64_12.dll`, `cudnn_ops64_*.dll`). These checks are skipped until the user explicitly requests CUDA.【F:utils/models.py†L344-L392】
...
### Chat with Bot
After the tray icon appears you can launch SocialRobot from the management window or by saying “chat with <identity>” (for example, “chat with Vision” or “chat with Einstein”). The Vision, Einstein, and Reception personas still pause briefly for background preparation, but only `refresh_datetime_memory` performs work today—the legacy documentation ingestion helper now logs that it is disabled while we migrate to the Docling RAG agent. Date/time snapshots continue to refresh only when their content changes or the 24-hour cooldown elapses, and they log terminal-only status so operators know when the preflight finished. Manual “update documentation” requests from the hotkey flow or inside the chat window now print the skip message, while “update datetime” still forces the snapshot for the active identity. Once a bot is active the chat window opens with text-to-speech already enabled and the hands-free VAD listener disabled. The persona badge doubles as the VAD toggle (swapping with a deaf icon when the listener is off) and the adjacent speak/mute button controls audio playback. TTS keeps the window anchored in the bottom-right corner and tucks the floating logo against the transcript’s lower-right corner at roughly half scale; minimizing the chat window slides the logo back to the desktop’s bottom-right corner at full size until you restore the transcript. Muting TTS hides the logo while leaving the window’s size and placement untouched. You can mix the controls freely (for example, deaf+speak for typed prompts with spoken replies or icon+mute for live speech with silent responses). Holding the right Ctrl push-to-talk hotkey still pauses an active listener so the manual transcription capture can run without interference, and the listener resumes automatically when you release the key—even if the audio stream briefly closes while the hotkey is down. Spoken “goodbye <identity>” commands continue to end the conversation immediately, while typed versions only trigger the shutdown when the VAD listener is enabled. Subsequent “chat with …” phrases relaunch the requested identity, and the push-to-talk workflow recognises the same keyword set. Keyword detection remains fuzzy, so light punctuation (“goodbye, vision” or “goodbye receptionist”) and typical STT slips (“chat was vision” or “chat was receptionist”) still trigger the automation unless documentation calls for strict matching. Each identity (`third_party/social_robot/personas/<name>`) bundles its system prompt, default voice, LLM settings, and future memory storage. Refer to docs/bot_integration.md for prerequisites, environment overrides, and automation tips.
