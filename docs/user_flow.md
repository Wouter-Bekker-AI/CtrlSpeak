...
### Chat with Bot
After the tray icon appears you can launch SocialRobot from the management window or by saying “chat with <identity>” (for example, “chat with Vision” or “chat with Einstein”). The Vision, Einstein, and Reception personas now block on background memory ingestion before the window becomes interactive: `refresh_document_memory` hashes `README.md` plus every Markdown file under `docs/`, while `refresh_datetime_memory` captures the current local date, timezone, and locale hints. Each helper refreshes the identity’s vector store only when its content changed or the 24-hour cooldown elapsed, and logs terminal-only status so operators know when the preflight finished. Manual “update documentation” or “update datetime” requests from the hotkey flow or inside the chat window force the corresponding helper to run immediately for the active identity. Once a bot is active it now opens with voice mode already running: the chat window anchors itself to the top-right corner beside the floating logo, displays conversation history, accepts keyboard input, and plays replies aloud while the microphone toggle stays highlighted. Click the microphone button to switch to text-only chat; the window moves to the bottom-right corner (the traditional floating-logo spot), hides the logo, and keeps responses on screen without TTS playback. Any text sent while voice mode is active follows the same workflow as live audio, including keyword handling. Holding the right Ctrl push-to-talk hotkey pauses SocialRobot’s VAD listener so the manual transcription capture can run without interference, and the listener resumes automatically when you release the key—even if the underlying audio stream briefly closes while the hotkey is down. Click the button again to re-enable voice interaction and restore the top-right placement. Voice commands like “goodbye <identity>” still stop the current conversation instantly because the transcription server intercepts the utterance and tells CtrlSpeak to shut down the session before SocialRobot continues, but the chat window now stays open so the user can close it manually. Typing the same “goodbye <identity>” keyword in the text box leaves the session running—the phrase is logged as normal chat instead of triggering shutdown. Subsequent “chat with …” phrases relaunch the bot with the requested identity. The push-to-talk workflow (hold right Ctrl, speak, release) recognises the same keywords: if no bot is running Vision launches immediately instead of pasting the phrase, switching identities shuts down the previous session before starting the new one, and saying “goodbye <identity>” exits using the same teardown path as the tray menu. Keyword detection is intentionally fuzzy, so light punctuation (“goodbye, vision” or “goodbye receptionist”) and typical STT slips (“chat was vision” or “chat was receptionist”) still trigger the automation unless documentation calls for strict matching. Each identity (`third_party/social_robot/personas/<name>`) bundles its system prompt, default voice, LLM settings, and future memory storage. Refer to docs/bot_integration.md for prerequisites, environment overrides, and automation tips.
