...
- Any runtime feature (bots, LangGraph tools, CLI utilities, automation scripts, etc.) that creates or mutates files **must** resolve its targets via `utils.config_paths` so the work lands in AppData (`data_root`, `config_root`, or documented children). Packaged builds run as a single executable, so writing beside the binary or inside the source tree will fail—treat the installation directory as read-only once compiled.
...
- Keyword detectors must remain **fuzzy**. Match punctuation-insensitive and near-miss transcriptions (for example, “goodbye, vision” or “chat was receptionist”) unless a specification explicitly demands exact phrasing. Hard matching regressions risk stranding conversations when STT introduces minor substitutions.
