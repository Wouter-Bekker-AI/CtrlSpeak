...
- **Documentation placeholder** – The legacy Markdown ingestion pipeline has been retired while we prepare to adopt the Docling RAG agent. Calls to `refresh_document_memory` remain in place for compatibility, but they now log that documentation ingestion is disabled and continue immediately.
...
| --- | --- | --- | --- | --- |
| Reception | `reception` | Welcomes users, answers CtrlSpeak usage questions, and routes requests to the right specialist. | Keeps conversations friendly and steers people toward the right persona while we transition the documentation workflow to Docling. | No vision capture and no tool calling; delegates advanced requests to the Vision or Einstein personas. |
| Vision | `vision` | General-purpose helper for day-to-day requests. | Vision-enabled—can capture the screen or clipboard on request and delivers polished natural-language replies. Documentation retrieval is temporarily disabled while the Docling integration is in progress. | Tool calling remains disabled; for complex planning or actions it will escalate to Einstein. |
...
1. `utils.bot_integration.start_bot` acquires the identity lock and still calls `refresh_datetime_memory(<identity>)` so the temporal snapshot stays current. Documentation ingestion has been removed; the companion `refresh_document_memory` helper is now a thin shim that logs the skip while we migrate to the Docling RAG agent.
2. Terminal-only messages confirm when the datetime snapshot runs or when a documentation refresh request is ignored. The GUI and TTS layers remain silent.
3. Operators can continue to say or type “update documentation” or “update datetime” (including via the Ctrl hotkey workflow). The documentation keyword now prints a reminder that ingestion is disabled, while the datetime keyword still forces a fresh snapshot for the active persona.

The LangGraph memory orchestrator remains available behind the `use_langgraph_memory_orchestrator` setting (or the `CTRLSPK_USE_LANGGRAPH_MEMORY_ORCHESTRATOR=1` environment variable). It is no longer forced on automatically for the bundled personas, and with documentation retrieval disabled its primary job is coordinating Goose tool usage and structured conversation persistence when you explicitly enable it.
...
- **update documentation** – Still triggers `refresh_document_memory`, but the helper now logs that documentation ingestion is disabled while the Docling workflow is under construction.
...
> **Bundled personas:** The Vision, Einstein, and Reception personas no longer force-enable this flag. Set it manually if you want their turns to flow through the LangGraph orchestration pipeline.
