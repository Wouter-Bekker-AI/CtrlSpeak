...
def test_release_version_gate_matches_v05():
    assert release.read_app_version() == "0.5.0"
    assert release.check_version("v0.5.0") == "0.5.0"
    with pytest.raises(SystemExit, match="tag/version mismatch"):
        release.check_version("v0.5.1")
...
def test_generate_and_verify_complete_release_set(tmp_path, monkeypatch):
    ...
    release.verify(Namespace(directory=str(tmp_path), tag="v0.5.0"))
