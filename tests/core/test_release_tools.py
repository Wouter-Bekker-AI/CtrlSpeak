...
def test_release_version_gate_matches_current_patch():
    assert release.read_app_version() == "0.7.6"
    assert release.read_server_versions() == ("0.7.6", "0.7.6")
    assert release.check_version("v0.7.6") == "0.7.6"
...
def test_generate_and_verify_complete_release_set(tmp_path, monkeypatch):
    ...
    release.verify(Namespace(directory=str(tmp_path), tag="v0.7.6"))
