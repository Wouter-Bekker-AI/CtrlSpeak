...
def test_v07_pyinstaller_spec_uses_stable_artifact_and_platform_adapters() -> None:
    ...
    assert system.APP_VERSION == "0.7.5"
...
def test_desktop_launcher_template_and_appstream_metadata_are_consistent() -> None:
    ...
    assert release.attrib["version"] == "0.7.5"
...
def test_v073_windows_and_hotfix_release_metadata_are_consistent() -> None:
    ...
    assert "filevers=(0, 7, 5, 0)" in version_info
    assert "prodvers=(0, 7, 5, 0)" in version_info
    assert "StringStruct(u'FileVersion', u'0.7.5')" in version_info
    assert "StringStruct(u'ProductVersion', u'0.7.5')" in version_info
